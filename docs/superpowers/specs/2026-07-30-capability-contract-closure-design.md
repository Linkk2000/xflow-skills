...
不能只证明模板存在或 `contract lint` 能拒绝坏文件。必须让使用 Skill 的 AI 从自然语言开始生成一份完整契约，并验证它能够被独立审核者理解和继续演进。

新建契约回放：

1. 输入一段没有 YAML 字段提示的真实能力描述。
2. 检查 AI 是否先保留原始陈述、查找现有契约并逐项澄清能力边界。
3. 人工明确认可能力目的、合法状态、失败和验证。
4. AI 生成 `status: accepted-design` 的 `contract.yaml`；全新未发布能力从 `0.1.0` 开始。
5. 运行 `devctl contract lint`，确认 YAML 可安全解析、ID 和引用闭合、约束有验证覆盖。
6. 使用人工语义量表检查内容不是为通过 Schema 填写的空话。

人工语义量表至少判断：

- `purpose` 表达参与者价值而非组件、接口或数据库动作。
- 合法状态和约束可以判定真假。
- 每个关键交互同时描述成功、拒绝和失败后保持状态。
- 验证场景能够转化为测试或明确人工步骤。
- 工程投影区分权威表示与派生表示。
- 当前能力、阻塞问题和未来能力没有混用。
- 稳定 ID 不依赖标题、Issue、分支或技术实现。

版本演进回放使用同一份契约依次测试：

| 变化 | 预期 |
|---|---|
| 只澄清措辞，不改变合法行为 | 根契约和实际修改对象提升 `PATCH` |
| 新增向后兼容的可选交互或验证 | 根契约和新增/修改对象体现 `MINOR` |
| 改变已有合法状态、约束或失败语义 | 根契约和受影响对象提升 `MAJOR` |
| 修复违反现有契约的实现 bug | 契约及对象版本保持不变 |
| 只记录未来能力且不进入当前验证 | 当前行为版本不因伪实现而提升 |

每次演进都运行 `devctl contract diff`，检查：

- 未变化对象的 ID 和版本保持稳定。
- 变化对象拥有与语义影响匹配的版本提升。
- 删除或替换对象具有迁移、弃用或 `supersedes` 说明。
- 受影响的验证和工程投影被列入人工复核清单。
- 新 YAML 再次通过 `contract lint` 和追溯检查。

机械检查通过只证明结构和版本规则一致，人工语义量表通过才证明生成的契约具有可用质量。第一版不提供自动填充业务内容的 `contract scaffold`；Skill 负责引导编写，devctl 负责验证和差异分析。
...
- AI 能从真实自然语言生成一份通过机械检查和人工语义量表的 `contract.yaml`。
- 同一契约经过 PATCH、MINOR、MAJOR 和实现差距回放后，稳定 ID 与对象版本演进合理。
